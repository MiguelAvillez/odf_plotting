...
def plot():
    ...
    full_state_history_postfit = np.loadtxt(f"{dir}/stateHistoryPropagatedPostFit_{file_tag}.txt")
    ...
    if plot_position_error:
        ...
        ax.plot((time_history_prefit[:-10] - time_history_prefit[0]) / 86400,
                   np.linalg.norm(state_history_prefit[:-10,0:3] - state_history_spice[:-10,0:3], axis=1) / 1e3,
        ...
        # ax.plot((residuals_time_history[:-10] - residuals_time_history[0]) / 86400,
        #            np.linalg.norm(residuals_history[:-10,0:3], axis=1) / 1e3,
        #            lw=lw*2, ls="--", label="Post-fit (from residuals)")
    ...
    if plot_kepler_elements_error:
        ...
        for state_history, out_name in zip([state_history_prefit, state_history_postfit], ["preFit", "postFit"]):
            ...
            axs[1,2].plot((time_history_prefit[:-10] - time_history_prefit[0]) / 86400,
                          (true_longitude[:-10] - true_longitude_spice[:-10]) * 180 / np.pi, lw=lw)
